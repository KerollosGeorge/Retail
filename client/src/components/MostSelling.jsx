...
export const MostSelling = () => {
  ...
  const {
    ...
    queryKey: ["mostSellingProducts"],
    queryFn: async () => {
      const { data } = await axios.get("/api/product/topSelling");
      return data;
    },
    staleTime: 600000,
    cacheTime: 1800000,
  ...
  useEffect(() => {
    if (products?.length > 0) {
      ...
      products.forEach((p) => setStock(p._id, p.stock));
    }
  ...
  if (isLoading) return <p>Loading most-selling products...</p>;
  if (error)
    return <p className="text-red-500">Failed to load most-selling products</p>;

  return (
    <div className="w-full flex flex-col items-center mt-8">
      <div className="w-full max-w-6xl flex justify-between items-center px-4">
        <h2 className="text-3xl font-bold">Most Selling</h2>
